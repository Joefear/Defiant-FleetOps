...
def assert_only_slice7_changes(before, upgraded):
    """Precisely allow the activated FK, two entrypoints and three attributed new tables."""
    ...
    assert len(upgraded["functions"]) == len(before["functions"]) + 2
    assert set(before["triggers"]) <= set(upgraded["triggers"])
    assert len(upgraded["triggers"]) == len(before["triggers"]) + 3
