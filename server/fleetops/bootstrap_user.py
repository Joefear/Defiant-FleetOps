...
This separate command runs as fleetops_app under RLS, creates one initial user, and never
...
def bootstrap_user(engine: Engine, *, organization_id: UUID, username: str, password: str) -> UUID:
    ...
    try:
        with engine.begin() as connection:
            ...
            if connection.execute(select(users.c.id).limit(1)).first() is not None:
...
def bootstrap_from_environment(environment: Mapping[str, str] | None = None) -> UUID:
    ...
    settings = Settings(
        database_url=make_url(environment["FLEETOPS_APP_URL"]),
